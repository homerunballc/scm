			...
			vuebind = new Vue({
				  ...
				  methods:{					
					  selimg:function(event){
						  ...
						  if(image.files[0]){
							  alert(window.URL.createObjectURL(image.files[0]));	// 이미지를 만들어주는 함수
							  vuevar.inputtext="asdfghj";
							  this.imgpath =  window.URL.createObjectURL(image.files[0]);  // 이미지 저장 
						  }
					  }
				  }  
				});	
					...
					<div class="content" style="margin-bottom:20px;">
                        ...
                        <div id="vuevar">
							...
							<div id="vuecheck">
									...
									<br>
									<div v-for="(item,index) in checklistitem" v-if="checklistitem.length"> // 한건이라도 있으면 for문을 돌려라 인덱스값을 가지고 아이템으로
							...
							<div id="vueoption">
                                    ...
                                    <template v-for="key in selectList"> // list를 key로 가지고 온다
								    {{key.value}}
